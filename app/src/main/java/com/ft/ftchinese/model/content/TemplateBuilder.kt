...
internal fun aiTranslationDisclaimerHtml(
    story: Story,
    language: Language,
    isTraditionalCn: Boolean
): String {
    if (!story.isAiTranslation || language == Language.ENGLISH) {
        return ""
    }

    val message = if (isTraditionalCn) {
        "為了第一時間為您呈現此資訊，中文內容為AI翻譯，僅供參考。"
    } else {
        "为了第一时间为您呈现此信息，中文内容为AI翻译，仅供参考。"
    }

    return """<div class="ai-disclaimer-container">$message</div>"""
}
...
class TemplateBuilder(private val template: String) {
    ...
    fun withUserInfo(account: Account?): TemplateBuilder {
        ...
        val gUserType = when (account.membership.webPrivilegeTier) {
            Tier.PREMIUM -> "VIP"
            Tier.STANDARD -> "Subscriber"
            null -> null
        }
        ctx["<!-- AndroidUserInfo -->"] = """
<script>
...
(function() {
    try {
        ...
        var gUserType = ${jsQuoted(gUserType)};
        ...
        if (!window.gUserType && gUserType) {
            window.gUserType = gUserType;
        }
...
var androidUserAddress = ${addr.toJsonString()}
    ...
    fun withStory(
        ...
        ctx["{story-body}"] = body
        ctx["<!--disclaimer-->"] = aiTranslationDisclaimer(story)
    ...
    private fun aiTranslationDisclaimer(story: Story): String {
